...
async function run() {
   ...
   if (lckMatch) {
      ...
      blocks.forEach(block => {
         ...
         console.log("EpMatch:", epMatch ? epMatch[1] : "None", "WinnerMatch:", winnerMatch ? winnerMatch[1] : "None");
      });
   } else {
      const idx = content.indexOf("Last Chance Kitchen");
      console.log("Context around LCK:", content.substring(Math.max(0, idx - 50), idx + 50));
   }
}
run();
